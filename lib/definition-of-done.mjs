...
export const DEFINITION_OF_DONE_CRITERIA = [
  ...
  {
    id: 'happy-and-failure-paths',
    label: 'The happy path and at least two failure paths are implemented.',
  },
  {
    id: 'interaction-states',
    label: 'Loading, empty, validation, error, and success states are usable.',
  },
  {
    id: 'server-validation',
    label: 'Server-side validation enforces the same rules as the interface.',
  },
  {
  ...
  {
    id: 'auditability',
    label: 'Actor, timestamp, source, and before/after values are auditable.',
  },
  {
  ...
  {
    id: 'api-tests',
    label: 'API authorization and invalid-input paths have tests.',
  },
  {
    id: 'fictional-fixture',
    label: 'A fictional demo fixture exercises the workflow.',
  },
  {
    id: 'data-model-consumers',
    label:
  ...
  {
    id: 'quality-gate',
    label: 'Test, lint, type check, and production build pass.',
  },
  {
    id: 'documentation',
    label: 'README, demo script, roadmap status, and limitations are updated.',
  },
  {
    id: 'metric',
    label:
...
export function validateDefinitionOfDone(manifest, options = {}) {
  ...
  if (
    !isNonEmptyString(manifest.featureId) ||
    !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(manifest.featureId)
  ) {
  ...
  if (
    !isNonEmptyString(manifest.userStory) ||
    manifest.userStory.trim().length < 30
  ) {
    errors.push(
      'userStory must describe the contract-operations value in at least 30 characters.',
    );
  }
  ...
  for (const [index, criterion] of manifest.criteria.entries()) {
    ...
    if (!knownIds.has(criterion.id)) {
      errors.push(
        `${prefix}.id "${criterion.id ?? ''}" is not a recognized criterion.`,
      );
      continue;
    }
    ...
    if (!VALID_STATUSES.has(criterion.status)) {
      errors.push(
        `${criterion.id}: status must be complete, not_applicable, or pending.`,
      );
      continue;
    }
      ...
      if (
        ...
        for (const [evidenceIndex, evidence] of criterion.evidence.entries()) {
          ...
          if (
            ...
            errors.push(
              `${evidencePrefix} requires non-empty kind and ref values.`,
            );
            continue;
          }
          if (!VALID_EVIDENCE_KINDS.has(evidence.kind)) {
            errors.push(
              `${evidencePrefix}.kind "${evidence.kind}" is not recognized.`,
            );
            continue;
          }
          if (PATH_EVIDENCE_KINDS.has(evidence.kind)) {
            ...
            if (!path || !pathExists(path))
              errors.push(
                `${evidencePrefix} path does not exist: ${path || evidence.ref}`,
              );
          }
        }
      }
    ...
    if (
      ...
      (!isNonEmptyString(criterion.rationale) ||
        criterion.rationale.trim().length < 30)
    ) {
      errors.push(
        `${criterion.id}: not_applicable requires a specific rationale of at least 30 characters.`,
      );
    }
  }
  ...
  const qualityCriterion = manifest.criteria.find(
    (criterion) => criterion?.id === 'quality-gate',
  );
  if (qualityCriterion?.status === 'complete') {
    const references = (qualityCriterion.evidence ?? []).map(
      (item) => item?.ref,
    );
    for (const command of REQUIRED_QUALITY_COMMANDS) {
      if (!references.includes(command))
        errors.push(`quality-gate evidence must include "${command}".`);
    }
  }

  const documentationCriterion = manifest.criteria.find(
    (criterion) => criterion?.id === 'documentation',
  );
  if (documentationCriterion?.status === 'complete') {
    const references = (documentationCriterion.evidence ?? []).map((item) =>
      evidencePath(item?.ref ?? ''),
    );
    for (const document of REQUIRED_DOCUMENTS) {
      if (!references.includes(document))
        errors.push(`documentation evidence must include ${document}.`);
    }
    if (
      !references.some(
        (reference) =>
          /limit/i.test(reference) || reference === 'PORTFOLIO_CASE_STUDY.md',
      )
    ) {
      errors.push(
        'documentation evidence must identify where limitations were updated.',
      );
    }
  }
  ...
  if (!isRecord(manifest.metric)) {
    ...
    if (!isNonEmptyString(manifest.metric.name))
      errors.push('metric.name is required.');
    if (!isNonEmptyString(manifest.metric.value))
      errors.push(
        'metric.value is required and must include its unit or boundary.',
      );
    if (
      !Number.isInteger(manifest.metric.sampleSize) ||
      manifest.metric.sampleSize < 1
    ) {
    ...
    if (!isNonEmptyString(manifest.metric.source)) {
      ...
      errors.push(
        `metric.source path does not exist: ${evidencePath(manifest.metric.source)}`,
      );
    }
  }

  const metricCriterion = manifest.criteria.find(
    (criterion) => criterion?.id === 'metric',
  );
  ...
  if (manifest.status === 'complete') {
    ...
    if (summary.pending > 0)
      errors.push('A complete feature cannot contain pending criteria.');
    if (summary.accountedFor !== summary.total) {
      errors.push(
        `A complete feature must account for all ${summary.total} criteria.`,
      );
    }
  }
