...
export const PHASE_EXECUTION_STEPS = [
  {
    id: 'define',
    label:
      'Define the user story, business rules, non-goals, and acceptance criteria.',
  },
  {
    id: 'fixture-first',
    label: 'Create a positive fixture and at least one failure or edge case.',
  },
  {
    id: 'model',
    label: 'Design the smallest schema change and migration path.',
  },
  {
    id: 'business-logic',
    label:
      'Implement deterministic calculations and state transitions outside the UI.',
  },
  {
    id: 'api',
    label:
      'Add validation, authorization, transaction, storage-cleanup, and audit controls.',
  },
  {
    id: 'interface',
    label: 'Add the smallest complete workflow and clear review states.',
  },
  {
    id: 'test',
    label:
      'Cover logic, state, invalid input, authorization, rollback, and regression paths.',
  },
  { id: 'measure', label: 'Save a bounded phase metric and sample size.' },
  {
    id: 'demonstrate',
    label: 'Add the feature to the resettable fictional demo.',
  },
  {
    id: 'document',
    label:
      'Update product, demo, roadmap, limitation, and resume documentation.',
  },
];
...
export function validatePhaseExecution(manifest, options = {}) {
  ...
  if (
    !isNonEmptyString(manifest.phaseId) ||
    !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(manifest.phaseId)
  ) {
  ...
  if (!isRecord(manifest.startGate)) {
    ...
    if (previous !== null) {
      if (!isRecord(previous) || !isNonEmptyString(previous.phaseId)) {
        errors.push(
          'startGate.previousPhase must be null or identify a phase.',
        );
      } else {
        ...
        if (
          !isNonEmptyString(previous.evidence) ||
          !pathExists(evidencePath(previous.evidence ?? ''))
        ) {
          errors.push(
            'startGate.previousPhase must reference existing completion evidence.',
          );
        }
      }
    }
    ...
    for (const field of [
      ...
      if (!Array.isArray(manifest.startGate[field])) {
        ...
        manifest.startGate[field].length > 0
      ) {
  ...
  if (manifest.steps.length !== PHASE_EXECUTION_STEPS.length) {
    errors.push(
      `steps must contain all ${PHASE_EXECUTION_STEPS.length} execution steps in order.`,
    );
  }
  ...
  for (const [index, expectedStep] of PHASE_EXECUTION_STEPS.entries()) {
    ...
    if (!VALID_STEP_STATUSES.has(step.status)) {
      errors.push(
        `${expectedStep.id}: status must be pending, in_progress, complete, or not_applicable.`,
      );
      unfinishedStep ??= expectedStep.id;
      continue;
    }
    ...
    if (unfinishedStep && terminal) {
      errors.push(
        `${expectedStep.id}: cannot finish before the earlier "${unfinishedStep}" step.`,
      );
    }
    if (!terminal) unfinishedStep ??= expectedStep.id;

    if (step.status === 'complete')
      validateEvidence(step, expectedStep.id, pathExists, errors);
    if (
      step.status === 'not_applicable' &&
      (!isNonEmptyString(step.rationale) || step.rationale.trim().length < 30)
    ) {
      errors.push(
        `${expectedStep.id}: not_applicable requires a specific rationale of at least 30 characters.`,
      );
    }
  }
  ...
  if (summary.inProgress > 1)
    errors.push('Only one execution step can be in progress at a time.');
  if (manifest.status === 'complete' && summary.finished !== summary.total) {
    errors.push(
      'A complete phase must finish or explicitly mark not applicable all 10 execution steps.',
    );
  }
  if (
    manifest.status === 'draft' &&
